...
def calloption(spot_price,strike_price,maturity,volatility,interest_rate):
    ...
    d2_term = d1_term - (volatility * np.sqrt(maturity))
...
# print (calloption(data['price'][1],data['price'][0],89,data['volatility'][0],data['3 MO'][0]/100))
# print (putoption(data['price'][1],data['price'][0],89,data['volatility'][0],data['3 MO'][0]/100))
...
## PORTFOLIO 4
# First extract delta of the options
def delta_call(spot_price,strike_price,maturity,volatility,interest_rate):
    # Write delta
    d1_term = (np.log(spot_price/strike_price) + (interest_rate + (np.square(volatility)/2)) * maturity) / (volatility * np.sqrt(maturity))
    first_term = norm.cdf(d1_term) * spot_price
    return first_term

def delta_put(spot_price,strike_price,maturity,volatility,interest_rate):
    # Write delta
    d1_term = (np.log(spot_price/strike_price) + (interest_rate + (np.square(volatility)/2)) * maturity) / (volatility * np.sqrt(maturity))
    first_term = norm.cdf(d1_term) * spot_price
    return (first_term - 1)

data['portfolio4_return'] = data['pct_return']
